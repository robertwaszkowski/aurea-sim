...
def build_repository(*, aliases_path: Path, splits_path: Path, crosswalk_path: Path,
                     data_root: Path, output_path: Path) -> dict:
    """Build an active-duration donor repository from calibration data only."""
    ...
    return {"profiles": len(profiles), "samples": sum(len(item["calibration_samples_seconds"]) for item in profiles), "output": str(output_path), "audit": payload["provenance"]["audit"]}
...
def main() -> None:
    ...
    print(json.dumps(build_repository(aliases_path=args.aliases, splits_path=args.splits, crosswalk_path=args.crosswalk, data_root=args.data_root, output_path=args.output), indent=2))
